...
export function Hero() {
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center pt-16 bg-zinc-50 dark:bg-zinc-950 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        ...
        >
          ...
          <span className="px-4 py-2 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 text-sm font-medium tracking-wide transition-colors duration-300">
            {language === 'pl' ? 'Dostępny do współpracy' : 'Available for work'}
          </span>
        </motion.div>
        ...
        >
          {language === 'pl' ? 'Cześć, jestem ' : 'Hi, I am '}<span className="text-emerald-600 dark:text-emerald-500">{name}</span>
          <br />
          <span className="text-zinc-500 dark:text-zinc-400 text-xl md:text-2xl font-medium mt-2 block transition-colors duration-300">{role}</span>
        </motion.h1>
        ...
        >
          ...
          <div className="flex items-center gap-4 mt-4 sm:mt-0 sm:ml-4">
            <SocialLink href={github} icon={<Github className="w-6 h-6" />} label="GitHub" />
            <SocialLink href={linkedin} icon={<Linkedin className="w-6 h-6" />} label="LinkedIn" />
            <SocialLink href={`mailto:${email}`} icon={<Mail className="w-6 h-6" />} label="Email" />
          </div>
        </motion.div>
